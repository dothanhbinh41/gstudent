...
class ResultHomeworkView extends StatefulWidget {
  int duration;
  int total;
  int score;
  int avgTime;
  ...
  ResultHomeworkView({this.total, this.duration, this.score,this.isMark,this.avgTime,this.isTest});

  @override
  State<StatefulWidget> createState() => ResultHomeworkViewState(total: this.total, duration: this.duration, score: this.score,isMark:this.isMark,avgTime: this.avgTime);
}

class ResultHomeworkViewState extends State<ResultHomeworkView> {
  int duration;
  int total;
  int score;
  bool isMark;
  int avgTime;
  ResultHomeworkViewState({this.total, this.duration, this.score,this.isMark,this.avgTime});
      ...
      body: Column(
        children: [
                            ...
                            child: RichText(
                              ...
                              text: TextSpan(
                                children: <TextSpan>[
                                  ...
                                  TextSpan(text: '\n', style: DefaultTextStyle.of(context).style),
                                  TextSpan(text: time.inMinutes > 0 ? 'Phút' : 'Giây', style: DefaultTextStyle.of(context).style),
                                ],
                              ),
                            ),
                            bottom: 16,
            ...
            child: RichText(
              ...
              text: TextSpan(
                children: <TextSpan>[
                  ...
                  TextSpan(
                      text: 'Thời gian làm: ' + _printTime(time) + '\n',
                      style: ThemeStyles.styleNormal(
                        textColors: HexColor("#fffac7"),
                      )),
                  TextSpan(
          ...
          Container(
            ...
            child: widget.isTest ? Center(child:  GestureDetector(
              onTap: () {
                Navigator.of(context).pop();
              },
              child:   ButtonYellowSmall('ĐÓNG', textColor: HexColor("#e3effa")),
            ),) : Row(
              children: [
              ...
              Expanded(child:   GestureDetector(
                ...
                child: ButtonYellowSmall('TIẾP TỤC', textColor: HexColor("#e3effa"),  ),
              ))
              ],
            ),
          )
  ...
  String _printTime(Duration duration) {
    int sec = duration.inSeconds ;
    int min = duration.inMinutes ;
    if(min > 0){
      var _sec = sec - 60*(sec~/60);
      return min.toString()+"p:"+_sec.toString()+" s";
    }else{
      return sec.toString() +' giây';
    }
  }
}
