...
class HpProgressbar extends StatefulWidget{
  int total;
  int current;
  HpProgressbar({this.current,this.total, });
  @override
  State<StatefulWidget> createState() => HpProgressbarState(current: this.current,total: this.total, );

}

class HpProgressbarState extends State<HpProgressbar> {
  int total;
  int current;
  HpProgressbarState({this.current,this.total,});
  ...
  Widget build(BuildContext context) {
    return Container(
        ...
        child: Stack(
          children: [
            Positioned(child: Image(image: AssetImage('assets/images/hp_bar.png'),fit: BoxFit.fill,),top: 0,
              bottom: 0,
              right: 0,
              left: 0,),
            Positioned(child: HPProgressBar(direction: Axis.horizontal,currentValue: ((current/total)*100).toInt(),size: 12,),top: 2,
                bottom: 2,
                right: 4,
                left: 2),
            Positioned(
                child: textpaintingBoldBase(current.toString()+'/'+total.toString(), 10,Colors.white,Colors.black,2),
                bottom: 0,
                left: 24),
          ],
        )
...
class HPProgressBar extends StatefulWidget {
  HPProgressBar({
    ...
    const TextStyle(color: const Color(0xFFFFFFFF), fontSize: 12,fontFamily: 'SourceSerifPro',fontWeight: FontWeight.bold),
  })  : _borderRadius = borderRadius ?? BorderRadius.only(bottomLeft: Radius.zero,bottomRight: Radius.circular(24),topLeft: Radius.zero,topRight: Radius.zero,),
        super(key: key);
...
class _HPProgressBarState extends State<HPProgressBar>
  ...
  @override
  void didUpdateWidget(HPProgressBar old) {
    triggerAnimation();
          ...
          child: Container(
            decoration: BoxDecoration(
                gradient: LinearGradient(
                    ...
                    colors: [HexColor("#DA5D56"), HexColor("#C42E29")])),
          ),
        )
    );
...
class ParallelogramClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    final path = Path()
      ...
      ..lineTo(size.width > 16 ? size.width *0.95 : 0, size.height)
